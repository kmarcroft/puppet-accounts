...
describe 'accounts::user', :type => :define do
  ...
  shared_examples 'not_having_home_dir' do |user, home_dir|
    ...
    it {
      is_expected.not_to contain_file(home_dir.to_s).with({
      'ensure'  => 'directory',
      'owner'   => owner,
      'group'   => group,
      'mode'    => '0700'
    ...
    it {
      is_expected.not_to contain_file("#{home_dir}/.ssh").with({
      'ensure'  => 'directory',
      'owner'   => owner,
      'group'   => group,
      'mode'    => '0700'
    ...
    it {
      is_expected.not_to contain_file("#{home_dir}/.ssh/authorized_keys").with({
      'ensure'  => 'present',
      'owner'   => owner,
      'group'   => group,
      'mode'    => '0600'
  ...
  shared_examples 'having_home_dir' do |user, group, home_dir|
    ...
    it {
      is_expected.to contain_file(home_dir.to_s).with({
      'ensure'  => 'directory',
      'owner'   => owner,
      'group'   => group,
      'mode'    => '0700'
    ...
    it {
      is_expected.to contain_file("#{home_dir}/.ssh").with({
      'ensure'  => 'directory',
      'owner'   => owner,
      'group'   => group,
      'mode'    => '0700'
    ...
    it {
      is_expected.to contain_file("#{home_dir}/.ssh/authorized_keys").with({
      'ensure'  => 'present',
      'owner'   => owner,
      'group'   => group,
      'mode'    => '0600'
    ...
    it {
      is_expected.to contain_file("#{home_dir}/.hushlogin").with(
        'ensure' => 'absent'
      )
    }
  ...
  context 'invalid ensure' do
    ...
    it do
      ...
      end.to raise_error(RSpec::Expectations::ExpectationNotMetError, /expects a match for Enum/)
    end
  end
  ...
  context 'remove group with user\'s account' do
    ...
    it {
      is_expected.to contain_user('mickey').with(
        'ensure' => 'absent'
      )
    }
  ...
  context 'supply custom path to authorized_keys file' do
    ...
    it {
      is_expected.to contain_file('/home/foo/.ssh').with({
      'ensure'  => 'directory',
      'mode'    => '0700',
    }) }

    it {
      is_expected.to contain_file('/home/foo/.ssh/auth_keys').with({
      'ensure'  => 'present',
    }) }
  ...
  context 'provide ssh key options' do
    ...
    it {
      is_expected.to contain_file("/home/foo/.ssh/authorized_keys").with({
      'ensure'  => 'present',
      'owner'   => 'foo',
      'group'   => 'foo',
      'mode'    => '0600'
  ...
  context 'ssh key with empty comment' do
    ...
    it {
      is_expected.to contain_ssh_authorized_key('jane_ssh-rsa').with({
      'type' => 'ssh-rsa',
      'key'  => 'AAA',
      'user' => 'jane',
    })}
    ...
    it {
      is_expected.to contain_user('jane').with(
        'ensure' => 'present'
      )
    }
  end
  ...
  context 'ssh key with empty options' do
    ...
    it {
      is_expected.to contain_ssh_authorized_key('jake_ssh-rsa').with({
      'type' => 'ssh-rsa',
      'key'  => 'AAA-jake',
      'user' => 'jake',
      'options' => '',
    ...
    it {
      is_expected.to contain_user('jake').with(
        'ensure' => 'present'
      )
    }
  end
  ...
  context 'ssh key with options array' do
    ...
    it {
      is_expected.to contain_ssh_authorized_key('luke_key').with({
      'type' => 'ssh-rsa',
      'key'  => 'AAA-luke',
      'user' => 'luke',
      'options' => ['darth=vader', 'foo=bar'],
    ...
    it {
      is_expected.to contain_user('luke').with(
        'ensure' => 'present'
      )
    }
  end
  ...
  context 'umask' do
    ...
    context 'disabled by default' do
      it {
        is_expected.not_to contain_file_line("umask_line_profile_johndoe").with({
        'ensure' => 'present',
      ...
      it {
        is_expected.not_to contain_file_line("umask_line_bashrc_johndoe").with({
        'ensure' => 'present',
    ...
    context 'modify bash settings when enabled' do
      ...
      it {
        is_expected.to contain_file_line("umask_line_profile_johndoe").with({
        'ensure' => 'present',
        'path'   => "/home/johndoe/.bash_profile",
        'line'   => "umask 077",
        'match'  => '^umask \+[0-9][0-9][0-9]',
      ...
      it {
        is_expected.to contain_file_line("umask_line_bashrc_johndoe").with({
        'ensure' => 'present',
        'path'   => "/home/johndoe/.bashrc",
        'line'   => "umask 077",
        'match'  => '^umask \+[0-9][0-9][0-9]',
  ...
  context 'populate home directory' do
    ...
    it {
      is_expected.to contain_file(home.to_s).with({
      'ensure'  => 'directory',
      'owner'   => title,
      'group'   => title,
      'mode'    => '0700',
  ...
  context 'hushlogin' do
    ...
    it {
      is_expected.to contain_file('/home/foo/.hushlogin').with(
        'ensure' => 'file',
        'owner'  => 'foo'
      )
    }
  ...
  context 'optional ssh dir management' do
    ...
    it {
      is_expected.not_to contain_file('/home/foo/.ssh').with(
        'ensure' => 'directory',
        'owner'  => 'foo',
        'mode'   => '0700'
      )
  ...
  context 'change ownership of .ssh dir' do
    ...
    it {
      is_expected.to contain_file("#{home}/.ssh").with(
        'ensure' => 'directory',
        'owner'  => 'root',
        'group'  => 'root'
      )
    ...
    it {
      is_expected.to contain_file("#{home}/.ssh/authorized_keys").with({
      'ensure'  => 'present',
      'owner'   => 'root',
      'group'   => 'root',
      'mode'    => '0600'
